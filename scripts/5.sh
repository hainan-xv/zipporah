...
for data in bad; do
  ...
  mkdir -p $output_dir
  paste $score_dir/ngram.?? $score_dir/translation.??-?? | awk -v w=$ngram_weight '{print w*($1+$2)+($3+$4)}' > $output_dir/scores.$ngram_weight.txt
done

testbase=$working/$id/step-5/bad

paste $output_dir/scores.$ngram_weight.txt $working/$id/step-2/corpus/bad.clean.?? > $working/$id/step-5/pasted.$ngram_weight.txt
