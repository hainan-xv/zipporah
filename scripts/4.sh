...
for data in bad dev; do
  ...
  paste $score_dir/ngram.?? $score_dir/oov-rate.?? $score_dir/translation.??-?? | tee $output_dir/feats.txt | awk '{print NF}' | uniq -c
  echo END OF OUTPUT
...
paste $testbase/scores.txt $working/$id/step-2/corpus/bad.clean.?? > $working/$id/step-4/pasted.txt
